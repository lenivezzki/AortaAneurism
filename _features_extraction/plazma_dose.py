...
w = open('плазма_дозы.txt', 'w')

import re

def var1(text):
    return re.findall(r'плазма свежезамороженная из дозы крови ед изм мл объем мл кол во ед кол во доз \d+', text)
...
for line in f.readlines():
    ...
    if a[10] == 'ПРОТОКОЛ_ПЕРЕЛИВАНИЯ_КРОВИ_И_КОМПОНЕНТОВ_КРОВИ':
        if k == 0:
            #typ = re.findall(r'эрвзвесь', text)
