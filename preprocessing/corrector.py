...
class Error_Corrector(object):
    ...
    def is_in_dict(self, string):
        ...
        is_all_in_dict = []
        for item in words:
            if set(item.lower()).issubset(self.dict_):
                is_all_in_dict.append((''.join(item), 1))
            else:
                is_all_in_dict.append((''.join(item), 0))
    ...
    def full_corrector(self, string):
